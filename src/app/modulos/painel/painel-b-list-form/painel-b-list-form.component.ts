...
export class PainelBListFormComponent implements OnInit {

  title: string  = "Painel de Chamada da Unidade de Atendimento Cidade Jardim";
  paineis: Painel[];
  id: number;
  filaId: number;
  senha: string;
  ...
  listarChamadaB(){
    this.painelService.findByChamadaB().subscribe((chamada: any)=>{
      this.id = chamada.id;
  ...
  listarPainelB(){
    this.painelService.findByPainelB().subscribe(
